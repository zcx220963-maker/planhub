...
async def memory_load_node(state) -> dict:
    """
    记忆加载节点：加载短期记忆 + 长期记忆 + 用户画像

    加载内容：
    - 短期记忆：Redis 中最近 10 轮对话（TTL 7天）
    - 长期记忆：Chroma 语义检索 top-K 条（含用户画像注入）
    - 用户偏好：Redis 中的 user_preference 字段
    """
    ...
    # 2. 加载长期记忆（语义检索，含画像注入）
    try:
        from src.app.service.long_term_memory_service import get_long_term_memory_service
        ltm_service = get_long_term_memory_service()
        # 用用户原始输入作为检索 query（比 LLM 预处理后的语义更准确）
        ...
        if long_term_memory:
            print(f"[DEBUG] memory_load: 长期记忆命中 {len(long_term_memory)} 条")
    except Exception as e:
        print(f"[WARN] memory_load: 长期记忆加载失败: {e}")
...
async def memory_save_node(state) -> dict:
    """
    记忆保存节点：
    1. 保存短期记忆到 Redis（最近 10 轮，TTL 7天）
    2. 保存完整会话到 Redis（前端会话列表用）
    3. 提取长期记忆到 Chroma（带 conversation_context 减少重复）
    4. 每 50 条长期记忆触发一次画像提炼
    5. 流程结束清除 checkpoint
    """
    memory_bridge = MemoryBridge()
    session_id = state.get("session_id", "default")
    ...
    try:
        ...
        try:
            ...
            if user_input and agent_output and user_id != "anonymous":
                # 构造对话上下文：取短期记忆中最近 3 轮作为 context
                short_term = state.get("short_term_memory", [])
                context_lines = []
                for msg in short_term[-6:]:  # 最多 6 条 = 3 轮
                ...
                # 4. 每累积 50 条长期记忆，触发一次画像提炼
                try:
                    all_memories = ltm_service.get_all_memories(user_id)
                    if len(all_memories) > 0 and len(all_memories) % 50 == 0:
                        profile_updated = await ltm_service.synthesize_profile(user_id)
                        if profile_updated:
                            print(f"[DEBUG] memory_save: 用户画像已更新 user_id={user_id}")
                except Exception:
                    pass

        except Exception as e:
            print(f"[WARN] memory_save: 长期记忆提取失败: {e}")

        # 5. 流程结束，立即清除 checkpoint（不等 TTL）
            ...
            "execution_trace": [
                {
                    ...
                    "long_term_extracted": long_term_count,
